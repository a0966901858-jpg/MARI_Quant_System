classdef GBDTExpertAgent < handle
    ...
    methods
        ...
        function [Score_time_oof, Score_space_oof] = train_and_predict_oof_cross_sectional(obj, ...
                E_time_3D, E_space_3D, X_norm_18D, Macro_2D, Y_Cont_3D, Expert_Active, stream)
            
            if nargin < 8 || isempty(stream)
                s = obj.resolveStream();
            else
            ...
            final_train_idx = randsample(s, totalActive, min(totalActive, max_train_samples), true, sample_weights);
        ...
        % =========================================================
        function P_crash_oof = train_and_predict_oof_crash(obj, Macro_2D, Y_Crash_1D, stream)
            ...
            for k = 2:K
                ...
                if length(unique(val_crash_y)) > 1
                    try
                        ...
                        for b = 1:n_boot
                            b_idx = randsample(s, n_val, n_val, true);
        ...
        % =========================================================
        function explain_shapley(obj, X_query_raw, X_bg_raw, mode, stream)
