classdef BuildDecoupledExtractors
    ...
    % 升級：Phase 15.5 Stage 4 規格 (★ 空間專家 SpaceExpertMixMode 動態注入、
    %       支援 GCN-only 與 Dynamic 雙模式切換、連續回歸與 Soft-IC 損失庫)
    ...
    methods
        function obj = BuildDecoupledExtractors(config, totalFeats, archType)
            disp(' ⚙️ [NetworkFactory] 啟動雙軌特徵萃取器構建工廠 (Stage 4 空間混合模式支援版)...');
            ...
            % ★ Stage 4 核心：讀取空間專家混合模式組態 (預設 'gcn_only')
        ...
        function [net_time, net_space] = buildNetworks(obj)
            ...
            % ★ Stage 4 修正：將 SpaceMixMode 顯式傳入 GraphSpatialFusionLayer
