...
classdef FeatureEngineer < handle
    ...
    methods
        ...
        function [X_norm_3D, Prices_Active, Expert_Active, Dates_Active, AdjMatrix_3D] = process(obj, dataStruct)
            ...
            disp(' -> 執行特徵標準化 (Cross-Sectional Z-Score + Macro 滾動標準化)...');
            ...
            for t = 1:numDays
                active_idx = Expert_Active(t, :);
                if sum(active_idx) >= min_cs_samples
                    vals = X_raw_3D(t, cs_feat_indices, active_idx);
                    mu_cross = mean(vals, 3, 'omitnan');
                    std_cross = std(vals, 0, 3, 'omitnan') + 1e-8;
                    X_norm_3D(t, cs_feat_indices, active_idx) = (vals - mu_cross) ./ std_cross; 
                else
                    X_norm_3D(t, cs_feat_indices, active_idx) = 0;
                end
