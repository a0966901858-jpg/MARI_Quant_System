classdef Config < handle
    ...
    % 升級: Phase 15.5 深度表徵增強 ＋ 200 輪極限平坦收斂與解耦退火生產基準版
    % 核心演進:
    %   1. 【空間剪枝鎖定】全面關閉 DyGAT 空間圖神經網路與協整建圖，鎖定時序專家權重 100% (防訊號稀釋)[cite: 1]
    %   2. 【表徵容量強化】啟用特徵直通殘差通路 (Highway Residual)，解決深層平滑瓶頸[cite: 1]
    %   3. 【200 輪走平驗證】MaxEpochs 與 Patience 設為 200，實質關閉早停中斷，保留快照回滾
    %   4. 【解耦退火排程】固定 120 輪餘弦退火，超期 (121~200輪) 鎖定 min_lr 形成水平漸近線
    %   5. 【權重 EMA 平滑】實裝影子權重衰減率 (Beta=0.995)，徹底撫平驗證損失鋸齒反彈
    %   6. 【週期嚴格同構】統一 20D 月步進全域基準線 (Horizon=20, Embargo=20, HACLag=20, Stride=20)[cite: 1]
    %   7. 【雙重統計熔斷】集中控管 DSR >= 0.95 與 CSCV PBO <= 10% 安全閥門
    ...
    properties (SetAccess = private)
        ...
        ModelDir         % 存放 VQ-VAE, GBDT, DL 抽取器大腦權重檔目錄
    ...
    properties
        ...
        % =================================================================
        % 依據 P2-1 消融實驗與 Round 8a-v3 結構性缺陷診斷：
        ...
        % 故全面簡化為單軌穩健決策，消除低訊噪比非馬可夫環境下 PPO 產生的牛市現金拖累[cite: 1]。
        ...
        % =================================================================
        % 實證支持：20D 連續超額報酬排序具備顯著 Rank IC (+0.1319 ~ +0.1736)，
        % 且 20 日調倉達成「預測跨度、隔離期、調倉步進」完全同構，兼顧動能捕捉與抗滑價摩擦[cite: 1]。
        ...
        % --- 輸入層動態抗噪正則化 ---
        FeatureDropoutRate       = 0.12;  % 特徵欄位隨機遮蔽率 (平衡去噪與微弱訊號保留)[cite: 1]
        InputNoiseStd            = 0.015; % 輸入層高斯動態噪聲標準差 (模擬真實盤面滑價與微抖動)[cite: 1]
        VariationalDropRate      = 0.15;  % 時序循環 Dropout 率 (整條 Sequence 共享同一個遮蔽 Mask)[cite: 1]
        AttentionDropRate        = 0.10;  % 自注意力權重丟棄率 (抑制模型對特定歷史形態死記硬背)
        
        % --- 複合連續損失函數超參數 (Huber Robust Loss + Soft-IC) ---
        ...
        Top_K_Assets        = 13;     % 最佳持股集中度 (經 Phase 4 BO 最佳化判定)
        
        % =================================================================
        % 6. 防過擬合安全斷路器與統計驗證門檻 (DSR / PBO / RobustScore)
        % =================================================================
        DSR_Threshold         = 0.95;  % Deflated Sharpe Ratio 顯著性下限 (95% 信心水準)
        PBO_Threshold         = 0.10;  % CSCV 12,870 條路徑過擬合機率上限 (PBO <= 10%)
        RobustScore_Threshold = 0.0;   % 子窗口穩健分數下限 (Mean IR - 0.5*Std IR > 0)
        
        % =================================================================
        % 7. 強化學習演算法參數 (RL Hyperparameters)
        % =================================================================
    ...
    methods
        ...
        function obj = Config()
            ...
            % 若空間專家訓練為 false，強制關閉建圖與空間別名，並將時序權重剛性鎖定為 1.0[cite: 1]
        ...
        % =================================================================
        % 函數：loadBOParams (貝氏尋優參數熱更新與 DSR / PBO 統計熔斷校驗)
        % =================================================================
        function loadBOParams(obj)
            ...
            if exist(boPath, 'file')
                try
                    ...
                    % 縱深防禦 (Defense-in-Depth) 檢查 DSR 與 PBO 統計顯著性
                    has_dsr = isfield(data, 'dsr_val') && isfield(data, 'best_robust_score');
                    has_pbo = isfield(data, 'pbo_val');
                    
                    is_invalid = false;
                    if has_dsr
                        if data.dsr_val < obj.DSR_Threshold || data.best_robust_score <= obj.RobustScore_Threshold
                            is_invalid = true;
                        end
                    end
                    if has_pbo
                        if data.pbo_val > obj.PBO_Threshold
                            is_invalid = true;
                        end
                    end
                    
                    if is_invalid
                        warning(['⚠️ [Config] 檢測到 BO 參數未達統計顯著標準！\n' ...
                                 '⚠️ 拒絕載入病態或過擬合參數，強制退回生產基準配置。']);
                    ...
                    if isfield(data, 'best_params')
                        ...
                        fprintf(' ⚡ [Config] 成功注入通過 DSR/PBO 顯著性檢定之 Phase 4 BO 最佳化參數：\n');
