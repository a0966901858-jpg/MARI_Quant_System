...
%% 7. 產出視覺化圖表
fig = figure('Name', 'P2-3 PPO Ensemble Study', 'Color', 'w', 'Position', [100, 100, 1100, 750]);
...
title('對數淨值曲線對比 (Log Equity Curve: Ensemble vs Single Agents)', 'FontWeight', 'bold');
...
subplot(3, 1, 3);
plot(Dates_Active(valid_start_t:numDays-1), cash_curves(valid_start_t:numDays-1, 1) * 100, 'Color', '#D95319', 'LineWidth', 1.5, 'DisplayName', 'Ensemble Cash %'); hold on;
plot(Dates_Active(valid_start_t:numDays-1), cash_curves(valid_start_t:numDays-1, 2) * 100, 'Color', '#0072BD', 'LineWidth', 1.0, 'LineStyle', ':', 'DisplayName', 'Balanced Cash %');
plot(Dates_Active(valid_start_t:numDays-1), cash_curves(valid_start_t:numDays-1, 4) * 100, 'Color', '#77AC30', 'LineWidth', 1.0, 'LineStyle', '--', 'DisplayName', 'Conservative Cash %');
xline(Dates_Active(idx_OOS_start), '--k', 'LineWidth', 1.5);
title('持倉現金避險比例對比 (Dynamic Cash Ratio %)', 'FontWeight', 'bold');
ylabel('Cash (%)'); ylim([0, 105]); legend('Location', 'northwest'); grid on;

saveas(fig, fullfile(configObj.ResultDir, 'P2_3_PPO_Ensemble_Ablation.png'));
fprintf('📊 視覺化圖表已儲存至: %s\n', fullfile(configObj.ResultDir, 'P2_3_PPO_Ensemble_Ablation.png'));