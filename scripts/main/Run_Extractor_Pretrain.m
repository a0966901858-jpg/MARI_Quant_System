...
% -------------------------------------------------------------
% 圖 1：雙軌萃取器訓練與驗證 Loss 曲線
% -------------------------------------------------------------
fig_loss = figure('Name', 'Phase 2: Extractor Pretrain Loss', ...
    'Position', [100, 100, 1200, 500], 'Color', 'w', 'Visible', 'off'); 
set(fig_loss, 'InvertHardcopy', 'off'); % 確保存檔時背景不被反轉

% 子圖 1：時序專家
subplot(1, 2, 1);
plot(1:epochs, historical_loss_time, '-o', 'LineWidth', 1.8, 'Color', '#D95319', 'DisplayName', 'Train Loss'); hold on;
plot(1:epochs, val_loss_time, '-x', 'LineWidth', 1.8, 'Color', '#0072BD', 'DisplayName', 'Val (Multi-Regime) Loss');
yline(0.6931, '--', 'Color', [0.7 0 0], 'LineWidth', 1.2, 'DisplayName', 'Random Guess (ln 2)');

title('Time Expert (Transformer-LSTM) BCE Loss', 'FontSize', 12, 'FontWeight', 'bold', 'Color', 'k');
xlabel('Epochs', 'FontSize', 10, 'FontWeight', 'bold', 'Color', 'k');
ylabel('BCE Loss', 'FontSize', 10, 'FontWeight', 'bold', 'Color', 'k');
...
% 子圖 2：空間專家
subplot(1, 2, 2);
plot(1:epochs, historical_loss_space, '-o', 'LineWidth', 1.8, 'Color', '#D95319', 'DisplayName', 'Train Loss'); hold on;
plot(1:epochs, val_loss_space, '-x', 'LineWidth', 1.8, 'Color', '#0072BD', 'DisplayName', 'Val (Multi-Regime) Loss');
yline(0.6931, '--', 'Color', [0.7 0 0], 'LineWidth', 1.2, 'DisplayName', 'Random Guess (ln 2)');

title('Space Expert (DyGAT) BCE Loss', 'FontSize', 12, 'FontWeight', 'bold', 'Color', 'k');
xlabel('Epochs', 'FontSize', 10, 'FontWeight', 'bold', 'Color', 'k');
ylabel('BCE Loss', 'FontSize', 10, 'FontWeight', 'bold', 'Color', 'k');
legend('Location', 'northeast', 'TextColor', 'k', 'Color', 'w', 'EdgeColor', [0.8 0.8 0.8]);
...
exportgraphics(fig_loss, lossFigPath, 'Resolution', 300, 'BackgroundColor', 'white'); % 高解析度 300 DPI 匯出
fprintf(' 📊 Loss 曲線 (白底黑字) 已儲存至: %s\n', lossFigPath);

% -------------------------------------------------------------
% 圖 2：GCN 靜態關聯圖譜矩陣 (白底黑/深藍像素)
% -------------------------------------------------------------
...
% 色表：0 為純白背景 [1 1 1]，1（存在關聯邊）為深黑/深藍 [0 0.15 0.4]
