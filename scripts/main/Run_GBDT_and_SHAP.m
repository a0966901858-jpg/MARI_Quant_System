...
% 升級：Phase 15.5 生產基準版 (★ 統一 60D 基準線：60 日連續橫截面超額報酬 Z-Score 目標、
...
clear; clc; close all;
disp('=================================================================');
disp('🚀 [Phase 15.5] 啟動 GBDT 連續迴歸選股預測流與 SHAP 歸因分析管線 (60D 基準線版)');
disp('=================================================================');
...
% ★ 核心修改 1：選股目標視窗全面改為 60 日，嚴格對齊 Phase 2 Direction 2
horizon_stock = 60;  % 由原本的 5 改為 60
...
% ★ 核心修改 2：終端提示與標籤計算全面對齊 60D
disp('--- 步驟 3：構建 60 日連續橫截面超額報酬與 10 日累積崩盤護欄標籤 ---');
% 3.1 橫截面 60 日遠期連續超額報酬 Z-Score (與 Phase 2 預訓練目標函數嚴格同構)
...
% ★ 核心修改 3：動態對齊 60D 評估參數 (若 GBDTExpertAgent 包含此類屬性則自動注入)
if isprop(gbdt_agent, 'TargetHorizon'), gbdt_agent.TargetHorizon = horizon_stock; end
if isprop(gbdt_agent, 'EmbargoDays'),   gbdt_agent.EmbargoDays   = horizon_stock; end
if isprop(gbdt_agent, 'HACLag'),         gbdt_agent.HACLag         = horizon_stock; end
...
[Score_time_oof_sub, Score_space_oof_sub, P_crash_oos_sub] = gbdt_agent.predict_oos(...
...
P_time_all(idx_OOS, :)  = Score_time_oof_sub;
