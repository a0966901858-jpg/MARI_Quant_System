...
% 升級：Phase 15.5 (★ 平行池全域注入 mrg32k3a 獨立子串流、DSR 統計顯著性熔斷機制、
...
disp('=================================================================');
disp('🧠 [Phase 15.5] 啟動 CIO 總管動態路由貝氏超參數尋優 (mrg32k3a 平行子串流版)');
disp('=================================================================');
...
% 依 labindex 鎖定子串流索引，確保 UseParallel=true 時各 Worker 執行子窗口模擬具備完全可重現性
...
[~, best_excess_returns] = run_subwindow_simulation(best_params, Prices_IS, Opens_IS, Expert_IS, ...
    P_time_IS, P_space_IS, P_crash_IS, vol20_is, spy_idx, 21, size(Prices_IS, 1) - 2, tau_noise, configObj);

pct_days_guarded = mean(P_crash_IS > best_params.Guardrail_CrashProb) * 100;

fprintf('\n🏆 【Phase 15.5 貝氏最佳化候選結果】 🏆\n');
...
plot(sim_dates, cum_excess, 'Color', '#0072BD', 'LineWidth', 2.0, ...
    'DisplayName', sprintf('Candidate Setup (TopK=%d, Guard=%.2f)', best_params.Top_K_Assets, best_params.Guardrail_CrashProb));
hold on;
...
function [ir_val, excess_returns] = run_subwindow_simulation(params, ~, Opens, Expert, P_time, P_space, P_crash, vol20, spy_idx, start_t, end_t, tau_noise, config)
    ...
    for s_i = 1:steps
        ...
        % 3. 帶死區之連續縮放護欄 (Deadband Continuous Risk Scaling)
        p_c = P_crash(t);
        if p_c <= guard_low
            risk_scale = 0.0;
