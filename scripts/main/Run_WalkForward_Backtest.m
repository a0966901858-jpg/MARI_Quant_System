...
fig_wf = figure('Name', 'MARI Quant Walk-Forward Backtest (Phase 15)', ...
...
yline(opt_guard, '--r', sprintf('Hard Hedge Threshold (%.3f)', opt_guard), ...
    'LineWidth', 1.4, 'DisplayName', 'Hard Hedge');
yline(max(0, opt_guard - 0.10), ':r', sprintf('Scaling Start (%.3f)', max(0, opt_guard - 0.10)), ...
    'LineWidth', 1.2, 'DisplayName', 'Scaling Start');
xline(Dates_Active(idx_OOS_start), '--k', 'LineWidth', 1.5, 'Color', 'k');

title('Macro Timing Diagnosis: Crash Probability vs Continuous Guardrail', 'FontSize', 11, 'FontWeight', 'bold', 'Color', 'k');
