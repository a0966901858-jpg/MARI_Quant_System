...
%       消除機械性全現金失真、日頻滑價成本校正、Open-to-Open 嚴格撮合)
% 職責：執行嚴格的因果律回測，產出無縫的 IS/OOS 真實績效、交易軌跡與視覺化診斷報表
% =========================================================================
clear; clc; close all;

disp('=================================================================');
disp('🚀 [Phase 15.5] 啟動 MARI 嚴格前向滾動回測 (護欄雜訊死區修復版)');
disp('=================================================================');
...
tau_noise = prctile(non_crash_p, 75); % 擷取 75% 雜訊分位數
...
fprintf('[DEBUG] guard_high=%.4f guard_low=%.4f tau_noise=%.4f\n', guard_high, guard_low, tau_noise);
...
for t = valid_start_t : numDays - 2
    ...
    % ★ 核心修復：崩盤護欄死區縮放 (Deadband Risk Scaling)
    p_c = P_crash_smooth(t);
    if p_c <= guard_low
        risk_scale = 0.0; % 位於日常雜訊底線內，不進行去槓桿
    elseif p_c >= guard_high
        risk_scale = 1.0; % 超過硬熔斷閾值，100% 避險
    else
...
%% 7. 繪製機構級回測視覺化報表 (標準學術白底黑字格式)
disp('--- 步驟 6：生成機構級視覺化報表 (白底黑字) ---');
fig_wf = figure('Name', 'MARI Quant Walk-Forward Backtest (Phase 15 Calibrated)', ...
    'Color', 'w', 'Position', [100, 100, 1250, 1000], 'Visible', 'off');
set(fig_wf, 'InvertHardcopy', 'off');

% 子圖 1：對數累積淨值曲線 (IS vs OOS)
subplot(4, 1, 1);
plot(Dates_Active(valid_start_t:idx_OOS_start-1), log10(is_port), 'LineWidth', 1.6, 'Color', '#D95319', 'DisplayName', 'MARI (IS)'); hold on;
plot(Dates_Active(valid_start_t:idx_OOS_start-1), log10(is_spy), 'LineWidth', 1.4, 'Color', '#0072BD', 'DisplayName', 'SPY (IS)');
plot(Dates_Active(idx_OOS_start:numDays-1), log10(oos_port), 'LineWidth', 2.0, 'Color', '#A2142F', 'DisplayName', 'MARI (OOS)');
plot(Dates_Active(idx_OOS_start:numDays-1), log10(oos_spy), 'LineWidth', 1.5, 'Color', '#4DBEEE', 'DisplayName', 'SPY (OOS)');
...
title('Log-Scale Cumulative Equity Curve (MARI vs SPY)', 'FontSize', 11, 'FontWeight', 'bold', 'Color', 'k');
...
% 子圖 2：投資組合回撤幅度對比 (%)
subplot(4, 1, 2);
mari_dd_is  = (is_port - cummax(is_port)) ./ cummax(is_port) * 100;
spy_dd_is   = (is_spy  - cummax(is_spy))  ./ cummax(is_spy)  * 100;
mari_dd_oos = (oos_port - cummax(oos_port)) ./ cummax(oos_port) * 100;
spy_dd_oos  = (oos_spy  - cummax(oos_spy))  ./ cummax(oos_spy)  * 100;

area(Dates_Active(valid_start_t:idx_OOS_start-1), mari_dd_is, 'FaceColor', '#D95319', 'FaceAlpha', 0.35, 'EdgeColor', 'none', 'DisplayName', 'MARI Drawdown (IS)'); hold on;
area(Dates_Active(idx_OOS_start:numDays-1), mari_dd_oos, 'FaceColor', '#A2142F', 'FaceAlpha', 0.5, 'EdgeColor', 'none', 'DisplayName', 'MARI Drawdown (OOS)');
plot(Dates_Active(valid_start_t:idx_OOS_start-1), spy_dd_is, 'Color', '#0072BD', 'LineWidth', 1.2, 'DisplayName', 'SPY Drawdown (IS)');
plot(Dates_Active(idx_OOS_start:numDays-1), spy_dd_oos, 'Color', '#0072BD', 'LineStyle', ':', 'LineWidth', 1.2, 'DisplayName', 'SPY Drawdown (OOS)');
xline(Dates_Active(idx_OOS_start), '--k', 'LineWidth', 1.5, 'Color', 'k');
title('Portfolio Drawdown Comparison', 'FontSize', 11, 'FontWeight', 'bold', 'Color', 'k');
...
% 子圖 3：動態避險現金比例 (%)
subplot(4, 1, 3);
area(Dates_Active(valid_start_t:numDays-1), cash_ratios(valid_start_t:numDays-1) * 100, ...
...
ylabel('Cash Ratio (%)', 'FontSize', 9, 'FontWeight', 'bold', 'Color', 'k');
ylim([0, 105]);
set(gca, 'Color', 'w', 'XColor', 'k', 'YColor', 'k', 'LineWidth', 1.0, ...
    'GridColor', [0.85 0.85 0.85], 'GridAlpha', 0.8, 'FontName', 'Helvetica', 'FontSize', 9);
grid on; box on;
...
subplot(4, 1, 4);
plot(Dates_Active(valid_start_t:numDays-1), P_crash_smooth(valid_start_t:numDays-1), ...
    'Color', '#7E2F8E', 'LineWidth', 1.4, 'DisplayName', 'Smoothed P(Crash)'); hold on;
yline(guard_high, '--r', sprintf('Hard Hedge Threshold (%.3f)', guard_high), ...
    'LineWidth', 1.4, 'DisplayName', 'Hard Hedge');
yline(guard_low, ':r', sprintf('Scaling Start Deadband (%.3f)', guard_low), ...
    'LineWidth', 1.2, 'DisplayName', 'Scaling Start');
yline(tau_noise, '-.b', sprintf('IS Noise Floor \\tau_{noise} (%.3f)', tau_noise), ...
