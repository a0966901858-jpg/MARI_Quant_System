% =========================================================================
% 腳本：2_Run_Extractor_Pretrain_SMOKE.m (Stage 2: 三大替代假說煙霧消融測試)
% 升級：Phase 15.5 Stage 0/1/2 統一規範版
...
disp('=================================================================');
disp('🧪 [Stage 2 SMOKE TEST] 啟動三大替代方案消融測試 (GPU 批次加速版)');
disp('=================================================================');
...
Adj_sub       = single(AdjMatrix_3D(sub_tickers, sub_tickers, sub_days));
...
clear X_norm_3D Prices_Active Expert_Active AdjMatrix_3D Dates_Active;
...
% 執行 GICS 產業內 Z-Score 標準化 (退縮保護門檻: 4 檔)
...
for t = 1:numDays
    ...
    if sum(act_m) >= 10
        ...
        for s = 1:length(unique_sectors)
            ...
            if strcmp(s_name, 'Unknown') || strcmp(s_name, 'Macro') || strcmp(s_name, 'Safe Haven'), continue; end
...
%% 3. 構建多 Horizon 目標標籤 (5D 與 60D 連續/二元)
disp('--- 步驟 3：構建 5D 與 60D 遠期連續報酬與 Beat-the-Median 標籤 ---');
...
for g = 1:num_groups
    ...
    % 提取逐日驗證期損失與線性探針指標
    [daily_val_losses, probe_metric, p_hac] = evaluate_probe_and_hac(net_t, X_in, Y_in, ...
        Expert_sub, val_days, seqLen, sample_T, grp.type, use_gpu);
    ...
    history_results(g).p_hac        = p_hac;
    history_results(g).type         = grp.type;
end

%% 6. 統計顯著性檢定與 Stage 2 結論報告
disp('====================================================================================================');
disp('📊 【Stage 2 三大替代方案消融檢定綜合報告】');
disp('====================================================================================================');
fprintf(' 實驗組別名稱                          | 最終 Val Loss | 探針評估 (AUC / IC) | HAC p-val | 對比 Base (p-val)\n');
fprintf('----------------------------------------------------------------------------------------------------\n');
...
for g = 1:num_groups
    ...
    if g == 1
        ...
        comp_str = sprintf('HAC p = %.4f', p_pair);
    end
    
    fprintf(' %-37s |    %.4f    |    %-16s |  %.4f   | %s\n', ...
        res.name, res.va_loss(end), metric_str, res.p_hac, comp_str);
end
fprintf('====================================================================================================\n\n');
...
if history_results(3).metric > 0.02 && history_results(3).p_hac < 0.05
    fprintf('  ⭐ 【DIRECTION 2 CONFIRMED】連續 Soft-IC 損失顯著優於二元分類，成功打破 ln(2) 停滯！\n');
else
    fprintf('  ✅ 【DIRECTION 2 FALSIFIED】連續回歸目標下 IC 仍貼近 0，證偽二元硬切中位數為主要瓶頸之假設。\n');
end

if history_results(4).metric > 0.03 && history_results(4).p_hac < 0.05
    fprintf('  ⭐ 【DIRECTION 3 CONFIRMED】60D 中長週期在動態 Embargo 隔離下具備真實因果預測力！\n');
else
    fprintf('  ✅ 【DIRECTION 3 FALSIFIED】60D 週期之高 ICIR 純屬未隔離重疊自相關造成之機械性假象。\n');
end
fprintf('====================================================================================================\n\n');

%% 7. 產出標準學術白底黑字視覺化圖表
fig = figure('Name', 'Stage 2 Smoke Ablation Study', ...
...
legend({'Raw 18D', 'GICS-Neutral'}, 'Location', 'northeast', 'TextColor', 'k', 'Color', 'w', 'EdgeColor', [0.8 0.8 0.8]);
set(gca, 'Color', 'w', 'XColor', 'k', 'YColor', 'k', 'LineWidth', 1.0, ...
    'GridColor', [0.85 0.85 0.85], 'GridAlpha', 0.8, 'FontName', 'Helvetica');
grid on; box on;

% 子圖 4：逐日驗證損失差盒狀分佈 (Base - Soft-IC)
subplot(2, 2, 4);
...
%% =====================================================================
% 輔助函數：逐日損失與線性探針 (AUC 或 Rank IC) 評估 (已消除未引用變數警告)
% =====================================================================
function [daily_losses, probe_metric, p_hac] = evaluate_probe_and_hac(net, X_in, Y_in, Expert, val_days, seqLen, ~, target_type, use_gpu)
    ...
    if strcmp(target_type, 'binary')
        ...
        % 逐日計算截面 Spearman Rank IC
        daily_ic = zeros(V, 1);
        v_idx = 0;
        row_c = 1;
        for i = 1:V
            n_act = sum(Expert(val_days(i), :));
            if n_act >= 10
                v_idx = v_idx + 1;
