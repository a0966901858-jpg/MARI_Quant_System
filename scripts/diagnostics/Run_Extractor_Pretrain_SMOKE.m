...
% 升級：Phase 15.5 Task A 修復版 (★ HAC max_lag 顯式貫穿、雙欄 p 值並列、IC 序列存檔)
...
% 僅載入必要特徵，移除未使用的 AdjMatrix_3D 避免 Linter 警告
...
for g = 1:num_groups
    ...
    % ★ Task A 核心更新：呼叫傳入 grp.H，接收 daily_ic 與兩種頻寬檢定值
    ...
    history_results(g).daily_ic     = daily_ic_g;    % 完整 IC 向量存檔，供後續深入分析
    history_results(g).metric       = probe_metric;
    history_results(g).p_hac        = p_hac_corr;    % 強制 lag = H 之檢定
    history_results(g).p_hac_auto   = p_hac_auto;    % 自動頻寬對照值
    history_results(g).hac_lag      = hac_lag_g;
    history_results(g).type         = grp.type;
end

% ★ 關鍵落盤：將包含完整 IC 序列的實驗結構體存檔，防止分析不可逆
...
for g = 1:num_groups
    ...
    if g == 1
        ...
        % 對比 Base 同樣套用當前組別的預測天數 H 作為 HAC lag 修正
...
% 因果鏈判讀：嚴格採用 Corrected HAC p 值判定
...
%% =====================================================================
% 輔助函數：逐日損失與線性探針 (★ Task A: 貫穿 H_horizon、回傳 IC 向量與雙重 p 值)
% =====================================================================
function [daily_losses, probe_metric, p_hac_corr, daily_ic_out, hac_lag_used, p_hac_auto] = ...
    ...
    if strcmp(target_type, 'binary')
        ...
        if length(daily_ic) >= 20
            ...
            % 強制頻寬下限檢定 (修正後: max(H, auto_bandwidth))
