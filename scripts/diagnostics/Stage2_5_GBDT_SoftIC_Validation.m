...
%       消除頻繁重建隨機引擎開銷、相容 RngStream 屬性綁定、
...
day_arr     = zeros(active_counts, 1, 'double'); % double 型別防範溢位
...
for s_idx = 1:num_seeds
    ...
    % 切換子串流序號，杜絕反覆實例化生成器的記憶體與排程開銷
    stream.Substream = s_idx;
    RandStream.setGlobalStream(stream);
    
    % ★ 核心相容綁定：同時支援 RngStream 與舊版 RandStream 屬性
    if isprop(trainer, 'RngStream')
        trainer.RngStream = stream;
    elseif isprop(trainer, 'RandStream')
