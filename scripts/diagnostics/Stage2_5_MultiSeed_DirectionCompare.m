...
% 升級：Phase 15.5 (★ mrg32k3a 獨立子串流調用 stream.Substream = s_idx、
%       消除頻繁重建隨機引擎開銷、5 組獨立子集跨體制檢驗、Wilcoxon 符號秩檢定)
% 職責：在 5 組獨立子串流 (n=5) 下評估 Direction 2 (Soft-IC) 與 Direction 3 (60D)，
%       並以 Wilcoxon 符號秩檢定 (Wilcoxon Signed-Rank Test) 檢驗跨子集穩健性
% =========================================================================
clear; clc; close all;

disp('=================================================================');
disp('🧪 [Stage 2.5 Task B] 啟動多種子/多子集穩健性交叉檢驗 (5 Substreams x 10 Epochs)');
disp('=================================================================');
...
%% 3. 啟動 5 組種子多子集訓練迴圈 (stream.Substream = s)
disp('--- 步驟 2：開始執行 5 組種子獨立子串流對照訓練 ---');
total_tic = tic;

for s = 1:n_seeds
    fprintf('\n=================================================================\n');
    fprintf('🌱 [Substream %d/%d] 切換獨立子串流 (Substream=%d)，抽樣 60 檔標的...\n', s, n_seeds, s);
    fprintf('=================================================================\n');
    
    % ★ 核心修復 2：直接切換子串流序號，杜絕反覆實例化隨機數引擎與排程開銷
    stream.Substream = s;
    RandStream.setGlobalStream(stream);
    
    sub_tickers = randsample(stream, candidate_pool, sample_T); % 使用獨立子串流抽樣
    ...
    for g_idx = 1:3
        ...
        if g_idx == 1
            ...
            fprintf('  [Substream %d] G1 Base BCE     -> Val Loss: %.4f | Probe AUC: %.4f\n', s, val_loss_final, probe_metric);
        elseif g_idx == 2
            res_g3_loss(s) = val_loss_final;
            res_g3_ic(s)   = probe_metric;
            res_g3_phac(s) = p_hac_corr;
            fprintf('  [Substream %d] G3 Soft-IC Loss -> Val Loss: %.4f | Rank IC: %+.4f (p=%.4f)\n', s, val_loss_final, probe_metric, p_hac_corr);
        else
            res_g4_loss(s) = val_loss_final;
            res_g4_ic(s)   = probe_metric;
            res_g4_phac(s) = p_hac_corr;
            fprintf('  [Substream %d] G4 60D Horizon  -> Val Loss: %.4f | Rank IC: %+.4f (p=%.4f)\n', s, val_loss_final, probe_metric, p_hac_corr);
        end
    end
end
...
disp(' ');
disp('====================================================================================================');
disp('📋 【Task B 核心產出表：5 組獨立子串流穩健性全量對照表】');
disp('====================================================================================================');
fprintf(' Substream | G1 Base Val Loss | G3 Soft-IC Val Loss | G1 - G3 損失差 | G3 OOF Rank IC | G4 60D OOF Rank IC (p-val)\n');
fprintf('----------------------------------------------------------------------------------------------------\n');
loss_diff_g1_g3 = res_g1_loss - res_g3_loss;
for s = 1:n_seeds
    fprintf('    #%d     |      %.4f      |       %.4f        |    %+.4f    |    %+.4f     |    %+.4f (p=%.4f)\n', ...
        s, res_g1_loss(s), res_g3_loss(s), loss_diff_g1_g3(s), res_g3_ic(s), res_g4_ic(s), res_g4_phac(s));
end
fprintf('----------------------------------------------------------------------------------------------------\n');
...
if p_wilcoxon_g1_g3 < 0.05
    fprintf('    - 判定: ⭐ DIRECTION 2 ROBUST CONFIRMED (跨 5 組獨立子串流損失均顯著改善)！\n');
else
    fprintf('    - 判定: ⚠️ DIRECTION 2 MARGINAL (跨子串流差異未達顯著)。\n');
end

fprintf('  > 假說 2 (Direction 3 穩健性): G4 (60D) 預測 Rank IC 是否跨子串流顯著大於 0?\n');
fprintf('    - 跨子串流平均 IC = %+.4f | Wilcoxon p-value = %.4f\n', mean(res_g4_ic), p_wilcoxon_g4_ic);
if p_wilcoxon_g4_ic < 0.05 && mean(res_g4_ic) > 0.02
    fprintf('    - 判定: ⭐ DIRECTION 3 ROBUST CONFIRMED (60D 動態隔離具備穩定正向預測力)！\n');
else
    fprintf('    - 判定: ✅ DIRECTION 3 FALSIFIED (跨子串流 IC 邊際薄弱或不穩定)。\n');
end

fprintf('\n📢 【誠實聲明（Stage 3 規範）】\n');
fprintf('   n=5 組獨立子串流樣本仍屬於小樣本檢定，此結果旨在將結論由「n=1 無法排除抽樣運氣」\n');
fprintf('   提升至「具備初步子集穩健性證據」，並非宣稱已達大數絕對證明。\n');
fprintf('====================================================================================================\n');

%% 5. 儲存多種子檢定結果
summary_taskB = table((1:n_seeds)', res_g1_loss, res_g3_loss, loss_diff_g1_g3, res_g3_ic, res_g4_ic, res_g4_phac, ...
