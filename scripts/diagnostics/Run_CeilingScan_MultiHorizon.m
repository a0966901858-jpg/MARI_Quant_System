...
% 升級：Phase 15.5 Task A 方案 (★ 納入 60-90D 長週期、連續回歸 Rank IC、
%       HAC max_lag 顯式貫穿防偽顯著、顯式剔除 NaN/Inf、動態因果 Embargo=H 隔離)
% 職責：系統性掃描橫截面連續與分類訊號天花板，檢驗中長週期是否純屬自相關假象
% =========================================================================
clear; clc; close all;

disp('=================================================================');
disp('🚀 [Workstream A] 啟動多 Horizon 天花板掃描 (Task A HAC 貫穿修復版)');
disp('=================================================================');
...
% 子圖 3：最大絕對特徵 ICIR 與 HAC 顯著比例 (修正後)
subplot(3, 1, 3);
...
%% =====================================================================
% 區域函式：單一任務獨立執行體 (★ Task A: 貫穿 max_lag = max(H, auto_bandwidth))
% =====================================================================
function res = execute_single_task_stage2(t_info, Prices_Active, Expert_Active, X_norm_3D, total_tasks)
    ...
    % 2. 原始特徵截面 Spearman IC 與 HAC 檢定 (★ 呼叫點 #3: 顯式傳遞 max_lag)
    ...
    % 計算逐日 OOF Spearman Rank IC 與 HAC 檢定 (★ 呼叫點 #4: 顯式傳遞 max_lag)
