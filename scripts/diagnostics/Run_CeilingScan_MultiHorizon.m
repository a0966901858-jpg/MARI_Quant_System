...
% 升級：Phase 15.5 Stage 2 方案 (★ 納入 60-90D 長週期、連續回歸 Rank IC、
%       動態因果 Embargo=H 隔離、HAC 顯著性檢定、無 Linter 警告)
% 職責：系統性掃描橫截面連續與分類訊號天花板，檢驗中長週期是否純屬自相關假象
% =========================================================================
clear; clc; close all;

disp('=================================================================');
disp('🚀 [Workstream A] 啟動多 Horizon 天花板掃描 (Stage 2 連續回歸與動態隔離版)');
disp('=================================================================');
...
% ★ Stage 2 擴充：覆蓋極短頻 (1D) 至中長頻 (60D, 90D)
...
disp('--- 步驟 2：啟動平行天花板掃描 (包含連續回歸 OOF Rank IC 檢定) ---');
...
parfor task_i = 1:total_tasks
    ...
    res = execute_single_task_stage2(t_info, Prices_Active, Expert_Active, X_norm_3D, total_tasks);
...
disp(' ');
disp('====================================================================================================================');
disp('📊 【Workstream A: 多 Horizon 訊號天花板掃描綜合報告 (Stage 2 連續回歸與動態隔離版)】');
disp('====================================================================================================================');
...
fig = figure('Name', 'Workstream A Multi-Horizon Ceiling Scan (Stage 2)', ...
...
title('Percentage of HAC Significant Features (p < 0.05)', 'FontSize', 11, 'FontWeight', 'bold', 'Color', 'k');
...
disp('=================================================================');
disp('🎯 [Workstream A] Stage 2 天花板掃描完成！');
disp('=================================================================');
...
%% =====================================================================
% 區域函式：單一任務獨立執行體 (Stage 2: 動態 Embargo=H + 連續回歸 Rank IC)
% =====================================================================
function res = execute_single_task_stage2(t_info, Prices_Active, Expert_Active, X_norm_3D, total_tasks)
    H        = t_info.H;
    ...
    % 3. 展平特徵與時序樣本 (最大抽樣 80,000 筆防 OOM)
    ...
    if active_counts > 80000
        rng(42, 'twister');
        sample_idx  = sort(randperm(active_counts, 80000));
    ...
    for k = 1:k_folds
        ...
        if sum(train_mask_k) > 500 && sum(test_mask_k) > 50
            ...
            % B. 連續回歸模型 (Ridge 嶺回歸，用於計算 OOF Rank IC)
            mdl_reg = fitrlinear(X_flat(train_mask_k, :), Y_cont_flat(train_mask_k), 'Learner', 'leastsquares', 'Regularization', 'ridge');
    ...
    % Day-Block Bootstrap 計算 AUC 信賴區間
    ...
    for b_i = 1:boot_n
        sample_d = randsample(u_eval_days, n_eval_days, true);
    ...
    for d_i = 1:n_eval_days
        ...
        if sum(d_idx) >= 10
            daily_oof_ic(d_i) = corr(eval_pred_c(d_idx), eval_true_c(d_idx), 'Type', 'Spearman', 'Rows', 'complete');
    ...
    if length(daily_oof_ic) >= 20
        [~, p_hac_oof_ic] = hac_significance_test(daily_oof_ic);
    else
        p_hac_oof_ic = 1.0;
    end
