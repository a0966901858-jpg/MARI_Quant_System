...
% 升級：Phase 15.5 Stage 2.5 方案 (★ 四軌乾淨對照: Time vs 2-Layer GCN vs 1-Layer GCN-only vs 1-Layer Dynamic、
...
disp('=================================================================');
disp('🧠 [Round 8a-v3] 啟動空間專家幾何容量與注意力解耦隔離診斷 (4 軌對照版)');
disp('=================================================================');
...
params_2l_gcn.W1 = dlarray(randn(64, numExtractorFeats, 'single') * sqrt(2 / numExtractorFeats));
params_2l_gcn.b1 = dlarray(zeros(64, 1, 'single'));
params_2l_gcn.W2 = dlarray(randn(64, 64, 'single') * sqrt(2 / 64));
params_2l_gcn.b2 = dlarray(zeros(64, 1, 'single'));
params_2l_gcn.W_aux = dlarray(randn(64, 1, 'single') * 0.01);
...
W_aux_1l_gcn = dlarray(randn(64, 1, 'single') * 0.01);
b_aux_1l_gcn = dlarray(zeros(1, 1, 'single'));
W_aux_1l_dyn = dlarray(randn(64, 1, 'single') * 0.01);
