...
%       Open-to-Open 權重漂移撮合、動態波動摩擦成本、mrg32k3a 隨機串流鎖定)
% 職責：全面量化 VQ-VAE 離散量化降噪對特徵訊噪比、機器學習泛化度與實盤夏普之邊際貢獻
% =========================================================================
clear; clc; close all;

disp('=================================================================');
disp('🧪 [P2-4] 啟動 VQ-VAE 降噪器消融實驗 (60D 連續 Z-Score + HAC Rank IC 版)');
disp('=================================================================');
...
%% 3. 模型層級檢驗：GBDT 5-Fold Purged CV (LSBoost 連續迴歸與 OOF Rank IC)
disp('--- 步驟 3：模型層級泛化度評估 (5-Fold Purged CV LSBoost OOF Rank IC) ---');
...
for k = 2:K
    ...
    if length(tr_idx) > 120000
        tr_idx = randsample(stream, tr_idx, 120000, false);
    end
    
    if length(tr_idx) > 500 && length(va_idx) > 50
        % LSBoost 連續迴歸訓練
        mdl_d = fitrensemble(X_flat_denoised(tr_idx, :), Y_flat(tr_idx), 'Method', 'LSBoost', ...
            'Learners', t_tree, 'NumLearningCycles', 30, 'LearnRate', 0.1);
        mdl_r = fitrensemble(X_flat_raw(tr_idx, :), Y_flat(tr_idx), 'Method', 'LSBoost', ...
            'Learners', t_tree, 'NumLearningCycles', 30, 'LearnRate', 0.1);
        
        oof_preds_denoised(va_idx) = single(predict(mdl_d, X_flat_denoised(va_idx, :)));
        oof_preds_raw(va_idx)      = single(predict(mdl_r, X_flat_raw(va_idx, :)));
        eval_mask_samples(va_idx)  = true;
    end
end

% 逐日橫截面 Spearman Rank IC 結算
eval_days_unique = unique(day_array(eval_mask_samples));
...
for d = 1:n_eval_days
    ...
    d_m = (day_array(eval_mask_samples) == day_v);
    if sum(d_m) >= 10
        valid_eval_cnt = valid_eval_cnt + 1;
        y_d = Y_flat(eval_mask_samples);
        y_d = y_d(d_m);
        
        pd_d = oof_preds_denoised(eval_mask_samples);
        pd_d = pd_d(d_m);
        
        pr_d = oof_preds_raw(eval_mask_samples);
        pr_d = pr_d(d_m);
        
        daily_ic_denoised(valid_eval_cnt) = corr(pd_d, y_d, 'Type', 'Spearman', 'Rows', 'complete');
        daily_ic_raw(valid_eval_cnt)      = corr(pr_d, y_d, 'Type', 'Spearman', 'Rows', 'complete');
    end
end
