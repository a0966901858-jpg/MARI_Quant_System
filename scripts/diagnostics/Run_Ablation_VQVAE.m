...
% 升級：Phase 15.5 (★ 特徵 ICIR + GBDT OOF AUC + 策略回測三層對照、HAC 顯著性檢定、白底黑字)
...
% 邊界保護裁切
actual_total = idx - 1;
if actual_total < totalActive
    X_flat_denoised = X_flat_denoised(1:actual_total, :);
    X_flat_raw      = X_flat_raw(1:actual_total, :);
    Y_flat          = Y_flat(1:actual_total);
    row_mapping     = row_mapping(1:actual_total, :);
    totalActive     = actual_total;
end
...
for i = 1:totalActive
    ...
    if t_i >= 1 && t_i <= numDays && tic_i >= 1 && tic_i <= numT
...
title('對數淨值曲線對比 (Log Equity: VQ-VAE Denoised vs Raw Unquantized)', ...
...
% -------------------------------------------------------------
% 子圖 3：5D Horizon ICIR 因子穩定性對比
% -------------------------------------------------------------
subplot(3, 1, 3);
b = bar([icir_5d_raw', icir_5d_vq'], 'grouped');
...
title('5D Horizon ICIR 因子穩定性對比 (5D ICIR Comparison)', ...
    'FontSize', 11, 'FontWeight', 'bold', 'Color', 'k');
ylabel('5D ICIR', 'FontSize', 9, 'FontWeight', 'bold', 'Color', 'k');
