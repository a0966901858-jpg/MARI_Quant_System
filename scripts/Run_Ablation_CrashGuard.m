...
%% 6. 產出視覺化圖表
fig = figure('Name', 'P2-2 Crash Guard Ablation Study', 'Color', 'w', 'Position', [100, 100, 1100, 750]);

subplot(3, 1, 1);
plot(Dates_Active(is_curve_idx), log10(port_curves(is_curve_idx, 1)), 'Color', '#D95319', 'LineWidth', 1.5, 'DisplayName', 'Group A (Continuous)'); hold on;
...
title('對數淨值曲線對比 (Log Equity Curve)', 'FontWeight', 'bold');
...
area(Dates_Active(valid_start_t:numDays-1), cash_curves(valid_start_t:numDays-1, 1) * 100, ...
    'FaceColor', '#77AC30', 'FaceAlpha', 0.5, 'DisplayName', 'Group A Continuous Cash Ratio'); hold on;
plot(Dates_Active(valid_start_t:numDays-1), P_crash_smooth(valid_start_t:numDays-1) * 100, ...
    'Color', '#A2142F', 'LineWidth', 1.2, 'DisplayName', 'P(Crash) %');
yline(opt_guard * 100, '--r', sprintf('Guardrail Threshold (%.1f%%)', opt_guard*100), 'LineWidth', 1.2);
xline(Dates_Active(idx_OOS_start), '--k', 'LineWidth', 1.5);
title('避險現金比例與平滑崩盤機率 (Crash Probability & Cash Allocation)', 'FontWeight', 'bold');
ylabel('Ratio (%)'); ylim([0, 105]); legend('Location', 'northwest'); grid on;

saveas(fig, fullfile(configObj.ResultDir, 'P2_2_CrashGuard_Ablation.png'));
fprintf('📊 視覺化圖表已儲存至: %s\n', fullfile(configObj.ResultDir, 'P2_2_CrashGuard_Ablation.png'));